...
export default function CreateProfileModal({ open, onClose }: Props) {
  ...
  const resetForm = () => {
    setSex("unknown");
    setName({ title: "", first: [], last: [], maiden: "", suffix: "" }); // last: []
    setBirthDate({ year: undefined, month: undefined, day: undefined, circa: false, bc: false });
    setDeathDate({ year: undefined, month: undefined, day: undefined, circa: false, bc: false });
    setIsDeceased(false);
  };
...
const sortDuplicatesClient = (items: DuplicateHit[]) => {
  const toTuple = (d?: any): [number, number, number] | null => {
    ...
    // pentru sort ASC: lună/zi lipsă la sfârșitul anului
  ...
  const nameKey = (it: DuplicateHit) => {
    const f = normFirst(it.name);
    const l = normLast(it.name);
    return `${f} ${l}`.trim();
  };

  // preferăm birth; dacă lipsește, folosim death
  const dateKey = (it: DuplicateHit): [number, number, number] | null =>
    toTuple(it?.birth?.date) || toTuple(it?.death?.date) || null;

  // sort global alfabetic după (first + last);
  // pentru nume egale: fără date primele, apoi cu date în ordinea datei (ASC)
  const cmp = (a: DuplicateHit, b: DuplicateHit) => {
    const na = nameKey(a);
    const nb = nameKey(b);
    ...
    if (da === null && db === null) {
      // tie-break stabil (opțional): după id/tree_ref
      const ka = a.id || a.tree_ref || "";
      const kb = b.id || b.tree_ref || "";
      return ka < kb ? -1 : ka > kb ? 1 : 0;
    }
    if (da === null) return -1; // fără dată înaintea celor cu dată
    if (db === null) return 1;

    // ambele au dată -> ASC (cel mai vechi primul)
    if (da[0] !== db[0]) return da[0] - db[0];
    if (da[1] !== db[1]) return da[1] - db[1];
    if (da[2] !== db[2]) return da[2] - db[2];

    // ultim tie-break (stabil)
    const ka = a.id || a.tree_ref || "";
    const kb = b.id || b.tree_ref || "";
    return ka < kb ? -1 : ka > kb ? 1 : 0;
  };
